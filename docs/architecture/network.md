...
- **Uptime Kuma** (LXC 103): Realiza checks periódicos de disponibilidad (ping/HTTP/DNS) sobre todos los servicios de la red. Esta configurado de manera que cada 10 minutos haga un chequeo de los servicios configurados.
...
- **Repositorio Enterprise activo sin suscripción:** Proxmox viene configurado por defecto con el repositorio `pve-enterprise`, que requiere una suscripción de pago. Al no tener suscripción, `apt-get update` fallaba y el dashboard mostraba el aviso "No hay una suscripción válida".

  ![Aviso de suscripción no válida](../screenshots/network/suscripcion-no-valida-proxmox.png)

  Revisando **Sistema → Repositorios**, se confirmó que el repositorio `pve-enterprise` estaba activado (`Enabled: true`) apuntando a `enterprise.proxmox.com/debian/pve`, y no había ningún repositorio `pve-no-subscription` configurado.

  ![Repositorio Enterprise activado sin suscripción](../screenshots/network/suscripcion-no-valida-proxmox-desactivar.png)

  **Solución:** se desactivó el repositorio `pve-enterprise` y se añadió el repositorio `pve-no-subscription`, gratuito y mantenido por la comunidad. Tras actualizar paquetes de nuevo, la actualización se completó sin errores. El aviso amarillo restante ("El repositorio no-subscription no es recomendado para uso en producción") es normal y esperado — es solo informativo, no un error.
...
**Solución:** en Nginx Proxy Manager, dentro de la configuración del host `server.traore.home`, activar la opción **"Websockets Support"**. Tras esto, la consola volvió a funcionar con normalidad accediendo por el dominio interno.
